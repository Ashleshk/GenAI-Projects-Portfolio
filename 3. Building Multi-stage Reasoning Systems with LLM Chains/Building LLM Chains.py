...
# MAGIC %md
# MAGIC
# MAGIC # Building Multi-stage Reasoning Systems with LLM Chains
# MAGIC
...
#####################################
# 2 We connect an LLM for Hyde, (we could use a slightly more advanced model 'gpt-3.5-turbo-instruct' since we have some more logic in this prompt).
...
# MAGIC ## Summary
# MAGIC
# MAGIC In this demo, we developed two AI systems using multi-stage reasoning. The first system, called **JekyllHyde**, was designed as a self-moderating tool for social media. One language model (LLM) generated comments for blog posts, while another LLM checked these comments for negative sentiment. The second system, named **DaScie**, followed the ReAct paradigm (thought-action-observation). It accepted plain text instructions and conducted data science analysis on data stored in a vector database.
# MAGIC
# MAGIC Using LLM chains we constructed sequential chains using multiple LLMs to perform multi-stage reasoning analysis. With `langchain` agents, we demonstrated how to build semi-automated systems with agents that can peform internet searches and generate content based on the search results.
