...
# MAGIC
# MAGIC Pinecone is a cloud-based vector database that offers fast and scalable similarity searchs for high-dimensional data, with a focus on simplicity and ease of use. 
...
# MAGIC * You must install the **Pinecone connector**.
# MAGIC * You must install the **Pinecone client**.
# MAGIC
# MAGIC These requirements will be covered below.
# MAGIC

# COMMAND ----------

# MAGIC %md
# MAGIC ### Installing the Pinecone Connector
# MAGIC
# MAGIC Since we will be interacting with Spark by writing a Spark dataframe out to Pinecone, we need a Spark Connector. You need to attach a Spark-Pinecone connector to your cluster using the steps below. For more information, refer to this [documentation](https://docs.pinecone.io/docs/databricks#setting-up-a-spark-cluster).. 
# MAGIC
# MAGIC 1. Open the configuration page for your cluster.
# MAGIC 1. In the **Libraries** tab, click **Install New**
# MAGIC 1. For **Libary Source**, choose *File Path/S3*.
# MAGIC 1. For **Libary Type**, choose *JAR*.
# MAGIC 1. For **Libary Path**, specify **`s3://pinecone-jars/spark-pinecone-uberjar.jar`**.
# MAGIC 1. Click **Install**.
# MAGIC
# MAGIC Note that this procedure only needs to be done once for each cluster. If you restart this cluster, the libary will be reinstalled automatically.
...
# MAGIC %md
# MAGIC For Pinecone, we need to generate the embeddings first and save them to a dataframe, before we can write it out to Pinecone for indexing. 
# MAGIC
...
# MAGIC 2. Using Spark Dataframe and use pandas UDFs to help us apply the embedding model to batches of data
...
# MAGIC %md
# MAGIC Note: The Pinecone free tier only allows one index. If you have existing indices, you need to delete them before you create a new index.
# MAGIC
# MAGIC We specify the similarity measure, embedding the vector dimension within the index.
# MAGIC
# MAGIC Read the documentation on how to [create an index here](https://docs.pinecone.io/reference/create_index/).
...
# MAGIC %md
# MAGIC Repeat the same step as in Method 1 above to delete and recreate the index. Again, we need to delete the index because the Pinecone free tier only allows one index.
# MAGIC
...
# MAGIC
# MAGIC **IMPORTANT!!** You need to attach a Spark-Pinecone connector `s3://pinecone-jars/spark-pinecone-uberjar.jar` to the cluster you are using. Otherwise, the following command would fail. Refer to this [documentation](https://docs.pinecone.io/docs/databricks#setting-up-a-spark-cluster) if you need more information. 
