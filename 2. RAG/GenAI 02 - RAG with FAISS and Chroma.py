...
# MAGIC
# MAGIC In general, a RAG architecture is composed of two main model types; the **retrieval model** and the **generation model.** In this demo, we will first demonstrate how to build a retrival model using both a vector library (`FAISS`) and a vector database (`Chroma`). Then, we will use the retrival results for the generation step. 
# MAGIC
# MAGIC In this notebook, we will implement the full workflow of RAG using text vectorization, vector search, and the question-answering workflow. While we use [FAISS](https://faiss.ai/) (vector library) and [ChromaDB](https://docs.trychroma.com/) (vector database), and a Hugging Face model, know that you can easily swap these tools out for your preferred tools or models!
# MAGIC
...
# MAGIC
# MAGIC 1. Implement a RAG end-to-end workflow (reading text, converting text to embeddings, and saving them to a vector store). 
# MAGIC 1. Query for similar documents using FAISS and ChromaDB 
# MAGIC 1. Apply a Hugging Face language model for question answering!
...
# MAGIC %md
# MAGIC
# MAGIC #### Saving embedding vectors to the FAISS index
...
# MAGIC %md
# MAGIC Tada! Now you can query for similar content! Notice that you did not have to configure any database networks beforehand or pass in any credentials. FAISS works locally with your code.
...
# MAGIC %md
# MAGIC ### Retrival with the Vector Database: Chroma
# MAGIC
...
# MAGIC %md
# MAGIC #### Step 1: Add data to the collection
# MAGIC
# MAGIC Since we are reusing the same data, we can skip the step of reading the data. As mentioned in the text above, Chroma can take care of text vectorization for us, so we can directly add text to the collection, and Chroma will convert the text into embeddings behind the scene. 
...
# MAGIC
# MAGIC Adding data to the collection will take some time to run, especially when there is a lot of data. In the cell below, we intentionally write only a subset of data to the collection to speed things up. 
...
# MAGIC
# MAGIC We will return the 10 most relevant documents. You can think of `10` as 10 nearest neighbors. You can also change the number of results returned as well. 
...
# MAGIC %md
# MAGIC #### Bonus: Add a filter statement
# MAGIC
# MAGIC In addition to conducting relevancy searches, we can also add filter statements. Refer to the [documentation](https://docs.trychroma.com/usage-guide#using-where-filters) for more information.
...
# MAGIC %md
# MAGIC
# MAGIC Yay, you have just completed the implementation of your first RAG architecture using text vectorization, search, and question answering workflow (that requires prompt engineering)!
# MAGIC
...
# MAGIC ## Summary
# MAGIC
# MAGIC In this demo, we demonstrated how to construct a complete Retrieval-Augmented Generation (RAG) model utilizing two distinct vector storage systems. Initially, we illustrated the utilization of FAISS, a vector library, to store embedding vectors and retrieve documents based on search terms. Subsequently, we explored the use of ChromaDB for document storage, including querying and updating data within ChromaDB. In the latter part of the demo, we employed the retrieval model's outcomes as context for text generation through prompt engineering.
